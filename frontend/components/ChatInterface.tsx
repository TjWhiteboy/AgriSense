...
import type { Message, SpeechRecognition, SpeechRecognitionEvent, SpeechRecognitionErrorEvent, User } from '../types';
  ...
  return (
    <div className="flex-1 flex flex-col bg-transparent overflow-hidden h-full">
          ...
          <div className="space-y-4">
            {messages.map((msg, index) => (
              <MessageBubble
                key={msg.id}
                message={msg}
                isStreamingTarget={isStreaming && !isLoading && index === messages.length - 1 && msg.role === 'model'}
              />
            ))}
      ...
      <div className="p-2 sm:p-4">
        ...
        <div className="relative flex items-end bg-white dark:bg-[#161B22] rounded-2xl p-2 shadow-lg transition-all duration-500 ease-in-out focus-within:ring-2 focus-within:ring-green-500 border border-gray-200 dark:border-gray-700">
          <button 
              onClick={handleAttachmentClick} 
              title={TRANSLATIONS.attachTooltip}
              className="text-gray-500 dark:text-gray-400 hover:text-green-500 dark:hover:text-green-400 p-2 rounded-full transition-colors duration-500 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed" 
              disabled={!isOnline}
          >
          ...
          <button 
              ...
              title={isRecording ? TRANSLATIONS.micTooltipStop : TRANSLATIONS.micTooltipStart}
              className={`p-2 rounded-full transition-colors duration-500 ease-in-out ${isRecording ? 'text-red-500 animate-pulse' : 'text-gray-500 dark:text-gray-400 hover:text-green-500 dark:hover:text-green-400'} disabled:opacity-50 disabled:cursor-not-allowed`}
              disabled={!isOnline}
          >
           ...
           <button 
              ...
              title={TRANSLATIONS.videoTooltip}
              aria-label={TRANSLATIONS.liveSession}
              className="p-2 rounded-full text-gray-500 dark:text-gray-400 hover:text-green-500 dark:hover:text-green-400 transition-colors duration-500 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={!isOnline}
          >
          ...
          <textarea
            ...
            placeholder={isOnline ? TRANSLATIONS.sendMessagePlaceholder : "You are offline. Type a message to send later."}
          ...
          <button
            ...
            title={TRANSLATIONS.sendTooltip}
            className="bg-gray-200 dark:bg-[#2D3340] text-gray-700 dark:text-white w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-md hover:bg-green-500 hover:text-white disabled:bg-gray-100 dark:disabled:bg-gray-600 disabled:cursor-not-allowed transition-all duration-500 ease-in-out shadow-sm hover:shadow-lg disabled:shadow-none"
