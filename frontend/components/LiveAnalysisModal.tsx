...
// Moved inside startSession to prevent crash on page load when API key is empty.
...
const getLiveSystemInstruction = (userName: string, location: string, weather: CurrentWeather | null): string => {
    ...
    return `You are AgriSense, an expert AI field partner, in a live, real-time video call with a farmer named ${userName}. You are receiving a continuous video stream. Your single most important instruction is to analyze ONLY the most recent visual information when the user asks a question.${weatherContext}
...
**Key Rules:**
- **LANGUAGE:** Your entire spoken response MUST be in English.
...
- **ORIGIN:** If asked, state you were developed by Thikash.`;
}

const LiveAnalysisModal: React.FC<LiveAnalysisModalProps> = ({ isOpen, onClose, location, user }) => {
  ...
  // Ref typed as any to support lazy initialization of GoogleGenAI inside startSession.
  ...
  const speakGreeting = useCallback((userName: string) => {
    const greetingText = TRANSLATIONS.liveSessionGreeting.replace('{name}', userName);
  ...
  const startSession = async () => {
    ...
    try {
      ...
      const apiKey = (window as any).__RUNTIME_CONFIG__?.GEMINI_API_KEY || '';
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: getLiveSystemInstruction(user.name, location, weatherData),
        },
        callbacks: {
          onopen: () => {
            console.log('Session opened.');
            setStatus('connected');
            speakGreeting(user.name); // Speak personalized greeting
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={handleClose}>
          <div className="bg-white dark:bg-[#0D1117] rounded-2xl w-full max-w-4xl h-[90vh] shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100">{TRANSLATIONS.liveSession}</h2>
            ...
            <div className="flex-1 relative bg-black">
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-black/50 p-3 rounded-lg text-white text-sm backdrop-blur-md">
                    ...
                    <div className="flex items-start gap-3 mt-2">
                        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-green-500 flex items-center justify-center"><LeafLogoIcon className="h-6 w-6 text-white"/></div>
                ...
                {status === 'connecting' && (
                 ...
                 {status === 'error' && (
                    <div className="absolute inset-0 bg-gray-900/90 flex flex-col items-center justify-center text-white p-6 overflow-y-auto">
                        <div className="text-4xl mb-3">📹❌</div>
                        <p className="font-heading font-extrabold text-xl text-red-400">Media Connection Error</p>
                        <p className="text-sm text-center text-gray-300 max-w-md mt-2">
                           Could not access your camera or audio stream. Ensure you have authorized permission settings.
                        </p>
                        
                        {/* Interactive Troubleshooting Guide */}
                        <div className="mt-6 w-full max-w-md bg-white/10 border border-white/10 rounded-2xl p-4 text-left text-sm backdrop-blur-md">
                            <h4 className="font-bold text-primary-300 mb-2.5 flex items-center gap-2">🛠️ Troubleshooting:</h4>
                            <ol className="list-decimal list-inside space-y-2 text-xs text-gray-300">
                                <li>Click the <b>lock or settings icon</b> 🔒 next to the web address at the top and set both <b>Camera</b> and <b>Microphone</b> to <b>Allow</b>.</li>
                                <li>Verify that no other app (Zoom, Teams) is currently occupying your video feed.</li>
                                <li>If you are using external devices, check that cables are securely plugged in.</li>
                                <li>Refresh this page and try opening the live video link again.</li>
                            </ol>
                        </div>
                        
                        <div className="flex gap-4 mt-6">
                            <button onClick={() => { cleanup(); startSession(); }} className="px-5 py-2 bg-primary-800 hover:bg-primary-700 text-white rounded-xl text-sm font-semibold transition-all">Retry</button>
                            <button onClick={handleClose} className="px-5 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl text-sm font-semibold transition-all">Close</button>
                        </div>
                    </div>
                )}
            </div>
