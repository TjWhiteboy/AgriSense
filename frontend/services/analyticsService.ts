...
const getSoilHealthForLocation = (location: string, hash: number, crop?: string): SoilHealth => {
    const districtData = DISTRICT_CROP_DATA[location];
    
    // Prioritize passed crop, then crops from the district if available, otherwise use the common list
    let primaryCrop = crop;
    
    if (!primaryCrop) {
        const cropSource = districtData?.crops.length > 0 ? districtData.crops : COMMON_CROPS;
        primaryCrop = cropSource[hash % cropSource.length];
    }
...
export const analyticsService = {
  ...
  getDashboardAnalytics: (location: string, selectedCrop?: string): DashboardAnalytics => {
    const hash = simpleHash(location);

    const pests = ['Aphids', 'Bollworm', 'Whitefly', 'Stem Borer', 'Thrips'];
    
    const districtCrops = DISTRICT_CROP_DATA[location]?.crops || COMMON_CROPS;
    const primaryCropForAnalytics = selectedCrop || districtCrops[hash % districtCrops.length];
    ...
    const soilHealth = getSoilHealthForLocation(location, hash, selectedCrop);
